...
class _NPYDataSource(FileDataSource):
    ...
    def collect_files(self):
        ...
        self.lengths = list(
            map(lambda l: int(l.decode("utf-8").split("|")[4]), lines))
...
def __train_step(device, phase, epoch, global_step, global_test_step,
    ...
    print('y:',y[:, :, :, 0].shape)
    loss = criterion(y_hat, y[:, :, :, 0], mask=mask)
...
def get_data_loaders(data_root, speaker_id, test_shuffle=True):
    ...
    for phase in ["train", "test"]:
        ...
        data_loader = data_utils.DataLoader(
            ...
            num_workers=1, sampler=sampler, shuffle=shuffle,
